...
def get_cnmp_plan(model, device):
    """
...
def main():
    rospy.init_node('cnmp_global_planner_node', anonymous=False)
    plan_pub = rospy.Publisher('cnmp_global_plan', Path, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        x_vals, y_vals = get_cnmp_plan(model, device)
